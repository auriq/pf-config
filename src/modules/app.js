...
// Extract PF remote name and bucket name, prefix from config
let pfRemoteName = '';
let pfBucketName = '';
let pfPrefix = '';
try {
  ...
  if (bucketMatch) {
    pfBucketName = bucketMatch[1].trim();
  }

  const prefixMatch = pfConfigContent.match(/prefix\\s*=\\s*([^\\n]+)/);
  if (prefixMatch) {
    pfPrefix = prefixMatch[1].trim();
  }
} catch (error) {
